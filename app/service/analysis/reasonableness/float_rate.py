# pricing_reasonableness/float_rate.py
"""
报价合理性 - 下浮率报价检查 Mixin

提供下浮率规则提取、逻辑表格/扁平文本中的下浮率行提取、
合规校验以及单一下浮率兜底逻辑。
"""
...
class FloatRateMixin:
    ...
    def _extract_float_rate_rules(self, section_text: str) -> Dict[str, Dict]:
        ...
        # 语义收紧
        if ("低于或等于所要求的下浮比例" in normalized_text or "低于或等于所要求下浮比例" in normalized_text) and "否决" in normalized_text:
            for key in rules:
                if rules[key]["op"] in {">=", ">"}:
                    rules[key]["op"] = ">"
        if ("高于或等于所要求的下浮比例" in normalized_text or "高于或等于所要求下浮比例" in normalized_text) and "否决" in normalized_text:
            for key in rules:
                if rules[key]["op"] in {"<=", "<"}:
                    rules[key]["op"] = "<"
    ...
    def _extract_float_rate_rows_from_row_table(self, tb: Dict) -> List[Dict]:
        ...
        for row in raw_rows[data_start:]:
            ...
            if non_tax_percents:
                ...
                small_non_tax = [t["value"] for t in tokens if 0 <= t["value"] <= 100 and t["value"] not in self.COMMON_TAX_RATES]
                if small_non_tax:
    ...
    def _extract_float_rate_rows_from_logical_tables(
        self, parsed: Dict, bid_page: Optional[int]
    ) -> List[Dict]:
        rows = []
    ...
    def _parse_flat_row_numbers(self, segment: str) -> Optional[Dict]:
        ...
        for m in token_matches:
            ...
            if val is None:
                continue
            values.append({"value": val, "has_percent": bool(m.group(2))})
        if len(values) < 3:
            return None
    ...
    def _extract_float_rate_rows(
        self, parsed: Dict, bid_page: Optional[int], bid_opening_text: str, rules: Dict[str, Dict]
    ) -> List[Dict]:
    ...
    def _extract_single_float_rate_from_table(
        self, parsed: Dict, bid_page: Optional[int], bid_opening_text: str
    ) -> Optional[float]:
    ...
    def _extract_single_float_rate_fallback(self, text: str) -> Optional[float]:
        ...
        for raw_line, line in zip(lines, normalized_lines):
            if any(
                k in line
                for k in ["大于", "高于", "不低于", "不少于", "低于", "小于", "不高于", "不大于", "等于", "否决", "否则"]
            ):
                continue
