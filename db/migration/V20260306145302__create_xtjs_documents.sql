...
COMMENT ON COLUMN xtjs_documents.identifier_id IS '文档UUID唯一标识，系统自动生成，作为主键和外键引用目标';
COMMENT ON COLUMN xtjs_documents.id IS '数据库内部自增数字序号，仅用于内部排序/排查，不作为外键或业务标识';
...
CREATE INDEX idx_xtjs_documents_active_create_time
    ON xtjs_documents (create_time DESC, identifier_id DESC)
    WHERE deleted = FALSE;

CREATE INDEX idx_xtjs_documents_active_type_create_time
    ON xtjs_documents (document_type, create_time DESC, identifier_id DESC)
    WHERE deleted = FALSE;
